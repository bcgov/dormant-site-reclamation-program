...
class ContractedWorkPaymentStatusChange(Base, AuditMixin):
    ...
    def __init__(self, contracted_work_payment, application, **kwargs):
        ...
        if self.contracted_work_payment_status_code == 'INFORMATION_REQUIRED':
            self.send_status_change_email(contracted_work_payment, application)

    def __repr__(self):
        return f'<{self.__class__.__name__} {self.contracted_work_payment_status_change_id}>'

    def send_status_change_email(self, contracted_work_payment, application):
