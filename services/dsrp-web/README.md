...
NOTE: Dormant Sites Reclamation Program is using the same Keycloak client as CORE, thus if the user is currently authenticated through CORE with an IDIR, the session will persist on Dormant Sites Reclamation Program.

This is a known issue, knowing that Ministry staff will not be interacting with Dormant Sites Reclamation Program and Proponents will not be interacting with CORE, it has been de-prioritized as it only affects the Dev team.

To avoid having permission issues:

- Do not have test CORE and test Dormant Sites Reclamation Program authenticated on the same browser
