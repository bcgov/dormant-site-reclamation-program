...
BEGIN	
	...
	UPDATE application SET json=jsonb_set(json, concat('{well_sites,', rec.well_index::int ,',contracted_work,', rec.contracted_work_type,'}')::text[],jsonb_set(json->'well_sites'->rec.well_index::int->'contracted_work'->rec.contracted_work_type, '{work_id}', CONCAT('"', rec.work_id::text, '"')::jsonb))
	WHERE guid=(rec.guid);
